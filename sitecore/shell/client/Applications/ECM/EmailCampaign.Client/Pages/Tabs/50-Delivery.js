...
function(
	...
	var deliveryTab = AppBase.extend({
		...
		sendButtons: ['StartTestButton', 'SendButton'],
		scheduleButtons: ['ScheduleTestButton', 'ScheduleButton'],
		activateButtons: ['ActivateMessageButton', 'ActivateTestButton'],

		initialized: function() {
			sitecore.trigger('mainApp', this);
			this.dispatchButtons = _.union(this.sendButtons, this.scheduleButtons, this.activateButtons);
			this.setMessageId();
		...
		attachEventHandlers: function() {
			sitecore.on({
					...
					'message:delivery:verification:failed': function () {
					    this.setControlsProperties(this.sendButtons, { 'isEnabled': true });
					}
				},
				this);
			...
			this.AutomaticTimeTextBox.on('change:text',
				function() {
			...
			this.VariantDataSource.on('change:isBusy',
				function() {
			...
			GlobalValidationService.on("validation:input:error", function () {
			    this.setControlsProperties(this.sendButtons, { isEnabled: true });
			}, this);
		},
		...
		attachScheduleEvents: function() {
			this.ScheduleTimezoneComboBoxDataSource.on('change:selectedItem',
				function() {
			...
			this.ScheduledTimeComboBoxDataSource.on('change:selectedItem',
				function() {
					var schedule = this.DispatchContext.get('schedule');
			...
			if (this.ScheduledEndTimeComboBoxDataSource) {
				this.ScheduledEndTimeComboBoxDataSource.on('change:selectedItem',
					function() {
						var schedule = this.DispatchContext.get('schedule');
						if (this.DispatchContext.get('hasSchedule') && schedule && schedule.endTime) {
							...
							comboBoxDataSource.setNearestTimeInComboBox(this.ScheduledEndTimeComboBoxDataSource,
								this.ScheduledEndTimeComboBox);
						}
					},
					this);
			}

			this.SendMessageNowRadioButton.on('change:isChecked',
				function() {
			...
			this.ScheduleDeliveryRadioButton.on('change:isChecked',
				function() {
			...
			if (this.ScheduleRecurringRadioButton) {
				this.ScheduleRecurringRadioButton.on('change:isChecked',
					function() {
		...
		addStatusMessage: function(status, statusDescription) {
			...
			var messageToAdd = {
				id: 'messagecontext.notification.statuschanged',
		...
		dispatch: function(actionName, isSchedule) {
			var isRecipientsValid = this.MessageContext.get('messageType') !== 'Triggered'
				? MessageValidationService.validateRecipients()
				: true;

			this.setControlsProperties(this.sendButtons, { isEnabled: false });
		...
		dispatchAction: function(actionName, requestParameters) {
			...
			sitecore.Pipelines.DispatchMessage.execute({
				app: this,
				currentContext: this.getDispatchRequestData(actionName, requestParameters)
			});
		},
		...
		doDispatch: function(actionName, isSchedule) {
			...
			if (isSchedule) {
				...
				if (this.MessageContext.get('messageType') === 'Triggered') {
					...
					return;
				} else if (typeof recurringSchedule !== 'undefined' &&
					recurringSchedule !== null &&
					recurringSchedule.hasRecurringSchedule === true) {
					if (typeof recurringSchedule.validation !== 'undefined' &&
						recurringSchedule.validation !== null &&
						recurringSchedule.validation.isValid === false) {
						Array.prototype.forEach.call(recurringSchedule.validation.errors,
			    ...
			    var okClicked = false,
					...
					on: {
					    ok: function () { okClicked = true },
					    // complete method will be executed for both "Cancel" or "x" button clicks
		...
		setIsEnablePreferredLanguage: function() {
			if (this.UsePreferredLanguageEnableCheckBox) {
				this.UsePreferredLanguageEnableCheckBox.set('isEnabled',
					!this.MessageContext.get('isReadonly') && this.DispatchContext.get('multiLanguageEnabled'));
			}
		},

		setIsEnableNotificationEmail: function() {
			if (this.NotificationEmailValueTextBox) {
				this.NotificationEmailValueTextBox.set('isEnabled',
					!this.MessageContext.get('isReadonly') && this.NotificationEnableCheckBox.get('isChecked'));
			}
		},
		...
		setupScheduleRadioButtons: function() {
			this.setControlsProperties(
				'ScheduleRecurringRadioButton',
				{ isChecked: this.DispatchContext.get('hasRecurringSchedule') },
				'ScheduleDeliveryRadioButton',
				{ isChecked: this.DispatchContext.get('hasSchedule') },
				'SendMessageNowRadioButton',
				{ isChecked: !this.DispatchContext.get('hasSchedule') && !this.DispatchContext.get('hasRecurringSchedule') }
			);
		},
		...
		refreshRecurringDailyPanel: function() {
			if (this.ScheduleRecurringDailyPanel.get('isLoaded')) {
				...
				this.ScheduleRecurringDailyPanel.viewModel.$el.hide('fast')
					.show('fast',
						_.bind(function() {
								this.ScheduleRecurringDailyPanel.refresh();
							},
