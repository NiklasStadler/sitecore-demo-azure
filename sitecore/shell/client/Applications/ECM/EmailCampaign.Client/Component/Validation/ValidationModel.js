...
function(
	...
	return backbone.Model.extend({
		...
		validateAll: function() {
			_.each(this.get('inputs'),
				_.bind(function(inputData) {
			...
			for (var rule in validators) {
				...
				if (rule === 'remote') {
					...
					if (!validatorMethod(value, this.getValidatorParams(validator), SpeakInput)) {
						if (!validator.silent) {
							this.addError(validator.message || ValidatorsService.messages[rule] || ValidatorsService.messages.default,
								messageId,
								SpeakInput);
						}
						return false;
		...
		removeError: function(messageId, SpeakInput) {
			var errors = _.filter(this.get('errors'),
				function(error) {
		...
		validateSender: function (emailAddress, indicator) {
            if (!emailAddress) {
                this.trigger('validation:indicator:success', 0, indicator);
                return;
            }

			ServerRequest(Constants.ServerRequests.VALIDATE_SENDER,
			{
				success: _.bind(function(response) {
						...
						this.trigger('validation:indicator:success', response.value, indicator);
					},
					this),
				async: true,
				type: 'get',
