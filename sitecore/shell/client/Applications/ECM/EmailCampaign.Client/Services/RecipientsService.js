define([    'sitecore',    'backbone',    '/-/speak/v1/ecm/constants.js',    '/-/speak/v1/ecm/ServerRequest.js',    '/-/speak/v1/ecm/MessageService.js'], function (    sitecore,    backbone,    constants,    ServerRequest,    MessageService    ) {    var ListModel = backbone.Model.extend({
        ...
        loadAllLists: function () {
            ServerRequest(constants.ServerRequests.LIST_MANAGEMENT_CONTACT_LIST, {
                data: {
                    pageSize: 0,                    pageIndex: 0,                    payLoad: 'full',                    filter: 'getRecentlyCreatedLists',
                    rootId: null,
        ...
        addList: function (type, list) {
            if (MessageService.messageContext.get('messageType') === "Triggered" &&
                list.Type === this.get('ignoredType')) {
                var messagetoAdd = {
                    id: "cannotAddSegmentedList",
                    text: sitecore.Resources.Dictionary.translate("ECM.Recipients.TriggeredMessageCanNotHaveSegmentedList"),
                    actions: [],
                    closable: true
                };
