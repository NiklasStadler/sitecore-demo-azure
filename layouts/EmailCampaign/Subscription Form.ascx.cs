...
namespace Sitecore.Modules.EmailCampaign.Layouts
{
  ...
  public class SubscriptionForm : UserControl
  {
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="SubscriptionForm"/> class. 
    /// Initializes a new instance of the <see cref="T:System.Web.UI.UserControl"/> class.
    /// </summary>
    ...
    public SubscriptionForm([NotNull] ILogger logger)
    {
      ...
      try
      {
        ...
        if (this.showList)
        {
          ...
          for (int i = 0; i < items.Count; i++)
          {
            ...
            {
              var listId = new ID(recipientListId).ToString();
              var info = this.SubscriptionInfos
                  .Cast<SubscriptionInfo?>()
                  .FirstOrDefault(x => x.HasValue && x.Value.ContactListId == listId && x.Value.UserSubscribed);

              if (info != null)
              {
                listsToUnsubscribe.Add(listId);
              }
            }
          }
        }
